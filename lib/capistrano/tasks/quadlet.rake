...
namespace :quadlet do
  ...
  task :disable do
    on roles(:app) do
      if test :systemctl, '--user is-active', fetch(:quadlet_container_service_name)
  ...
  task :restart do
    on roles(:app) do
      execute :loginctl, 'enable-linger'
      execute :systemctl, '--user', 'reload-or-restart', fetch(:quadlet_container_service_name)
    end
  end
...
after 'deploy:updated', 'quadlet:restart'
